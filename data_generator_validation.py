...
def generate_and_store_data(dim):
    ...
    #X = np.random.uniform(-10,10,size=(Config.sample_dim))
    X = np.random.normal(0, 0.4, dim)
    
    
    #y = 0.1*X+0.5*np.power(X,2)+0.2*np.power(X,3)+0.1*np.power(X,4)+0.01*np.power(X,5)
